...
export default function PairWorkspace() {
  ...
  async function mutate(action: () => Promise<unknown>, message: string) {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Could not update the pair alert.",
      );
    } finally {
      setBusy(false);
    }
      ...
      <div className="mx-auto max-w-[1440px] space-y-6 px-5 py-8 lg:px-10">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            Monitor divergence or convergence using aligned daily closes.
          </p>
        </div>
        {(error || refreshError) && (
          <div
        ...
        <div className="grid items-start gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
          <section className={panel}>
            <h2 className="mb-5 flex items-center gap-2 font-semibold">
              <GitCompareArrows size={18} className="text-emerald-400" />
            ...
            <form className="space-y-4" onSubmit={inspect}>
              <div className="grid grid-cols-2 gap-3">
                <label className="text-xs text-slate-400">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      change({ ticker_a: e.target.value.toUpperCase().trim() })
                    }
                    maxLength={15}
                ...
                <label className="text-xs text-slate-400">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      change({ ticker_b: e.target.value.toUpperCase().trim() })
                    }
                    maxLength={15}
                    pattern="[A-Z][A-Z0-9.\-]{0,14}"
                    required
              ...
              <label className="block text-xs text-slate-400">
                  ...
                  onChange={(e) =>
                    change(
                      e.target.value === "ratio"
                        ? { metric: "ratio", condition: "above", threshold: 1 }
                        : {
                            metric: "zscore",
                            condition: "outside",
                            threshold: 2,
                ...
                >
                  <option value="zscore">Log-spread z-score</option>
                  <option value="ratio">Price ratio · A ÷ B</option>
                </select>
              </label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <label className="text-xs text-slate-400">
                    ...
                    <input
                      ...
                      required
                      onChange={(e) =>
                        change({ lookback: Number(e.target.value) })
                      }
                    />
                  </label>
                  <label className="text-xs text-slate-400">
                    ...
                    <input
                      ...
                      required
                      onChange={(e) =>
                        change({ hedge_ratio: Number(e.target.value) })
                      }
                    />
                  </label>
                </div>
              ...
              <label className="block text-xs text-slate-400">
                ...
                <select
                  ...
                  onChange={(e) =>
                    change({
                      condition: e.target.value as PairConfig["condition"],
                    })
                  }
                >
                  <option value="above">At or above the level</option>
                  <option value="below">At or below the level</option>
                  {config.metric === "zscore" && (
                    <>
              ...
              <label className="block text-xs text-slate-400">
                ...
                <input
                  ...
                  min={
                    config.metric === "ratio"
                      ? 0.000001
                      : ["outside", "inside"].includes(config.condition)
                        ? 0
                        : -20
                  }
                  max={config.metric === "ratio" ? 100000 : 20}
                  disabled={busy}
                  required
              ...
              <label className="flex items-start gap-2 text-xs leading-relaxed text-slate-400">
                ...
                />
                Repeat after the condition clears on a later daily bar
              </label>
              ...
              <button
                ...
                onClick={save}
              >
                {editing ? "Save & rearm pair alert" : "Create pair alert"}
              </button>
              {editing && (
                <button
                  type="button"
            ...
            <p className="mt-4 text-xs leading-relaxed text-slate-500">
              ...
              A condition already met when armed triggers on the next worker
              check.
            </p>
          </section>
          <div className="min-w-0 space-y-6">
            <section className={panel}>
              ...
              {!preview ? (
                <p className="py-16 text-center text-sm text-slate-500">
                  Choose two stocks and preview their relationship before
                  creating an alert.
                </p>
              ) : (
                <>
                  <div className="mb-5 flex flex-wrap justify-between gap-3">
                    <div>
                      <h3 className="text-xl font-semibold">
                        {preview.ticker_a} / {preview.ticker_b}
                      </h3>
                      <p className="mt-1 text-xs text-slate-400">
                        {preview.source === "demo"
                          ? "SYNTHETIC DEMO"
                          : preview.source.toUpperCase()}{" "}
                        · Completed close {preview.as_of}
                      </p>
                    </div>
                    <span
                  ...
                  <div className="mb-5 grid grid-cols-3 gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-lg">{money(preview.price_a)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-400">
                        {preview.ticker_b} close
                      </p>
                      <p className="mt-2 text-lg">{money(preview.price_b)}</p>
                    </div>
                  </div>
                  <PairChart snapshot={preview} config={config} />
            ...
            <section className={panel}>
              ...
              <p className="text-xs leading-relaxed text-slate-400">
                {config.metric === "zscore"
                  ? `Spread = ln(${config.ticker_a || "A"}) − ${config.hedge_ratio} × ln(${config.ticker_b || "B"}). The latest spread is compared with the mean and population standard deviation of the preceding ${config.lookback} shared sessions, excluding the latest close. β is a fixed weight you choose; it is not automatically fitted.`
              ...
              <p className="mt-3 text-xs leading-relaxed text-slate-500">
                The background worker checks about every 30 seconds while the
                API runs. Alerts use completed daily closes, not intraday
                quotes. Mismatched dates, stale history, or an undefined z-score
                pause evaluation and show an error. Notifications appear here;
                no email, push, or broker orders are sent. A large z-score does
                not establish cointegration or predict a profitable trade.
              </p>
            </section>
          </div>
        </div>
        <section className={panel}>
          ...
          {!alerts.length ? (
            <p className="py-6 text-center text-sm text-slate-500">
              No pair alerts yet. Preview a pair and create your first rule.
            </p>
          ) : (
                ...
                <li key={a.id} className="py-4">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        {a.active ? <Pause size={15} /> : <Play size={15} />}
                      </button>
                      <button
                        ...
                        onClick={() =>
                          void mutate(
                            () =>
                              api(`pairs/alerts/${a.id}`, { method: "DELETE" }),
                  ...
                  {a.last_error && (
                    <p className="mt-2 text-xs text-red-300">{a.last_error}</p>
                  )}
        ...
        <section className={panel}>
          <h2 className="mb-4 font-semibold">Recent pair notifications</h2>
          {!events.length ? (
            <p className="py-4 text-sm text-slate-500">
              Triggered alerts will appear here and persist across restarts.
            </p>
          ) : (
            <ul className="max-h-80 divide-y divide-[#243141] overflow-auto">
              {events.map((event) => (
                ...
                >
                  <div>
                    <b className="text-sm">
                      {event.config.ticker_a} / {event.config.ticker_b}
                    </b>
                    <p className="mt-1 text-xs text-emerald-300">
                      {conditionLabel(event.config)} · Observed{" "}
