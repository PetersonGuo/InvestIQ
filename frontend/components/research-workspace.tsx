...
type Result = {
  ...
  assumptions: string[];
  logs: string;
  unfilled_final_signal: number | null;
};
type Run = {
  id: string;
  status: string;
...
export default function ResearchWorkspace() {
  ...
  useEffect(() => {
    ...
    if (symbol && /^[A-Z][A-Z0-9.\-]{0,14}$/.test(symbol)) setTicker(symbol);
    return () => {
      active = false;
    };
  }, []);
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        if (alive)
          setError(e instanceof Error ? e.message : "Could not load backtest.");
      }
    }
    void poll();
    return () => {
  ...
  function switchLanguage(next: Language) {
    ...
    setCode(
      drafts[next] ??
        templates.find(
          (t) => t.language === next && t.template_id === loadedPreset,
        )?.code ??
        "",
    );
  }
  ...
  function strategyInput(): Strategy {
    ...
    if (
      ...
      throw new Error("Parameters must be a JSON object with numeric values.");
    return { name, language, code, params: parsed };
  }
  async function save() {
    setError("");
    setBusy(true);
    try {
      ...
      setError(e instanceof Error ? e.message : "Could not save strategy.");
    } finally {
      setBusy(false);
    }
  }
  async function changeInterval(value: string) {
    ...
    setError("");
    if (value === "1d") { setStart(isoDate(365)); setEnd(isoDate(1)); return; }
    setStart(new Date(Date.now()-1800000).toISOString().slice(0,19));
    setEnd(new Date().toISOString().slice(0,19));
    setBusy(true);
    try {
      const snapshot = await api<{bars: {time: string}[]}>(`stocks/${encodeURIComponent(ticker)}?interval=${value}`);
      if (snapshot.bars.length) {
        setStart(snapshot.bars[0].time.slice(0,19));
        setEnd(new Date(Date.parse(snapshot.bars.at(-1)!.time)+intervalSeconds[value]*1000).toISOString().slice(0,19));
      }
    } catch (error) { setError(error instanceof Error ? error.message : "Could not load intraday window."); }
  ...
  async function execute() {
    ...
    try {
      ...
      const response = await api<Run & { cache_hit: boolean }>("backtests", {
        method: "POST",
        body: JSON.stringify(request),
      });
      setRun(response.status === "completed" ? response : null);
      if (response.cache_hit) setNotice("Cached result · identical inputs. Data snapshots refresh after one hour or on a new trading date.");
      ...
      <header className="border-b border-[#233040] bg-[#0d1620]">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-4 px-5 py-5 lg:px-10">
          ...
          <nav className="flex flex-wrap gap-5 text-sm">
            ...
            <Link href="/pairs" className="text-slate-400 hover:text-white">
              Pair alerts
            </Link>
          </nav>
        </div>
      ...
      <div className="mx-auto max-w-[1600px] space-y-6 px-5 py-8 lg:px-10">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-400">
            Find a candidate. Bring your code. Test it against historical
            prices.
          </p>
        </div>
        {error && (
        ...
        <div className="grid items-start gap-6 xl:grid-cols-[300px_minmax(0,1fr)]">
          <aside className="space-y-6">
            <section className={panel}>
              ...
              >
                <label className="block text-xs text-slate-400">
                  ...
                  >
                    <option value="HOT_BY_VOLUME">Active by volume</option>
                    <option value="TOP_PERC_GAIN">
                      Top percentage gainers
                    </option>
                    <option value="TOP_PERC_LOSE">Top percentage losers</option>
                  </select>
                </label>
                ...
                <button className={`${button} w-full`} disabled={scanning}>
                  {scanning ? "Scanning…" : "Scan US stocks"}
                </button>
              </form>
              <p className="mt-3 text-xs leading-relaxed text-slate-500">
                US major exchanges · USD stocks. Broker-ranked candidates, not
                trade recommendations.
              </p>
              {scanned && (
                <p className="mt-4 text-xs uppercase tracking-wide text-emerald-400">
                  {scanSource === "demo" ? "Synthetic demo scan" : "IBKR scan"}{" "}
                ...
                {candidates.map((c) => (
                  <li key={`${c.rank}-${c.ticker}`}>
                    <button
                      ...
                      onClick={() => {
                        setTicker(c.ticker);
                        setNotice(`${c.ticker} selected for backtesting.`);
                      }}
                    >
                      <span className="mr-2 text-xs text-slate-500">
                        {c.rank}
                      </span>
                      <b className="text-sm">{c.ticker}</b>
            ...
            <section className={panel}>
              <h2 className="mb-3 font-semibold">Saved strategies</h2>
              {!saved.length && (
          ...
          <div className="min-w-0 space-y-6">
            <section className={panel}>
              ...
              <div className="mb-5 rounded-xl border border-[#2b3948] bg-[#0c141e] p-4">
                ...
                <p className="mt-3 text-xs leading-relaxed text-slate-400">
                  {
                    templates.find((t) => t.template_id === presetId)
                      ?.description
                  }
                </p>
                <p className="mt-2 text-[11px] text-slate-500">
                  Loading replaces the editor, parameters, and warmup. These are
                  comparison baselines, not optimized strategies.
                </p>
              </div>
                ...
                <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-[#3c5266] px-3 py-2.5 text-xs">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      if (!file) return;
                      if (file.size > 64000) {
                        setError("Strategy code must be under 64 KB.");
                        return;
                      }
                      ...
                      setLanguage(file.name.endsWith(".py") ? "python" : "cpp");
                      setCode(text);
              ...
              <details className="mt-3 text-xs text-slate-400">
                ...
                <div className="mt-3 space-y-2 leading-relaxed">
                  ...
                  <p>
                    State persists across bars within one run. Signals fill at
                    the next bar’s open. Warmup calls build state but do not
                    place orders. Use imports or native libraries available in
                    the backend environment.
                  </p>
                  <p>
                    Run trusted code only. Strategies run locally in a separate
                    process with time limits; this is not a security sandbox.
                  </p>
                </div>
              </details>
              <form
              ...
              >
                <label className="block text-xs text-slate-400">Backtest interval<select className={`${field} mt-1`} value={interval} disabled={busy || running} onChange={e => void changeInterval(e.target.value)}>{intervals.map(value => <option key={value} value={value}>{value === "1d" ? "Daily" : value}</option>)}</select></label>
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <label className="text-xs text-slate-400">
                    {interval === "1d" ? "Start date" : "Start time · UTC"}
                    <input
                      className={`${field} mt-1`}
                      type={interval === "1d" ? "date" : "datetime-local"}
                      step={interval === "1d" ? undefined : 1}
                      required
                      value={start}
                  ...
                  <label className="text-xs text-slate-400">
                    {interval === "1d" ? "End date" : "End time · UTC (exclusive)"}
                    <input
                      className={`${field} mt-1`}
                      type={interval === "1d" ? "date" : "datetime-local"}
                      step={interval === "1d" ? undefined : 1}
                ...
                <label className="flex items-center gap-2 text-xs text-slate-400"><input type="checkbox" checked={forceRerun} onChange={(e) => setForceRerun(e.target.checked)} />Force rerun (skip result cache)</label>
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <p className="max-w-lg text-xs leading-relaxed text-slate-500">
                    Uses completed regular-session bars: up to 500 daily bars, or
                    50,000 intraday bars across 40 historical pages. Intraday
                    times are UTC; small bars have limited historical availability. No
                  ...
                  >
                    <Play size={15} />
                    {running ? "Backtest running…" : "Run backtest"}
                  </button>
                </div>
              </form>
            </section>
            {running && (
              ...
              >
                Loading historical data and executing strategy… You can review
                other runs while this finishes.
              </div>
            )}
            {run?.status === "failed" && (
            ...
            {result && (
              <section className={panel}>
                <div className="mb-5 flex flex-wrap justify-between gap-3">
                  <div>
                    <h2 className="text-lg font-semibold">
                      {run.request.ticker} · {run.request.name}
                    </h2>
                    <p className="mt-1 text-xs text-slate-400">
                      {run.request.language === "cpp" ? "C++" : "Python"} ·{" "}
                      {result.data.source.toUpperCase()} · {result.data.start}{" "}
                      to {result.data.end} · {result.data.bar_count} {result.interval ?? "1d"} bars
                    </p>
                  </div>
                ...
                <div className="mb-6 grid grid-cols-2 gap-3 lg:grid-cols-3">
                  {[
                    ["Final equity", money(result.metrics.final_equity ?? 0)],
                    [
                      "Total return",
                    ...
                    [
                      "Buy & hold",
                      formatPercent(result.metrics.benchmark_return_percent),
                    ],
                    [
                      "Max drawdown",
                      formatPercent(result.metrics.max_drawdown_percent),
                    ],
                    [
                      result.interval && result.interval !== "1d" ? "Sharpe · daily runs only" : "Sharpe · 0% risk-free",
                    ...
                    <div key={label} className="rounded-xl bg-[#0b141e] p-4">
                ...
                <div className="mt-6">
                  ...
                  {!result.fills.length ? (
                    <p className="py-5 text-sm text-slate-400">
                      No fills in this window. Check your signal conditions and
                      warmup.
                    </p>
                  ) : (
                      ...
                      <table className="w-full whitespace-nowrap text-left text-xs">
                        <thead className="text-slate-500">
                          <tr>
                            {[
                              ...
                              <th key={h} className="px-2 pb-3 font-normal">
                                {h}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {result.fills.map((fill, index) => (
                            ...
                            >
                              <td className="p-2">{fill.signal_date}</td>
                              <td className="p-2">{fill.time}</td>
                              <td
                                className={`p-2 uppercase ${fill.side === "buy" ? "text-emerald-300" : "text-orange-300"}`}
                              >
                                {fill.side}
                              </td>
                              <td className="p-2">{fill.quantity}</td>
                              <td className="p-2">{money(fill.price)}</td>
                              <td className="p-2">{money(fill.commission)}</td>
                              <td className="p-2">
                                {fill.realized_pnl == null
                                  ? "—"
                                  : money(fill.realized_pnl)}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                ...
                <details className="mt-5 text-xs text-slate-400">
                  ...
                  <p className="mt-3">
                    {result.metrics.open_shares} shares remain open at the final
                    close. Sell-fill win rate:{" "}
                    {formatPercent(result.metrics.win_rate_percent)}. Annualized
                    return:{" "}
                    {formatPercent(result.metrics.annualized_return_percent)}.
                  </p>
