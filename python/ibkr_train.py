...
DAYS_TO_TRAIN = int(os.getenv("DAYS_TO_TRAIN"))
...
def fetch_ibkr_data(ticker, end_date, whatToShow, retries=3, delay=60):
        ...
        for whatToShow in IBKR_WHAT_TO_SHOW.split():
            ...
            if not subset_df.empty:
                if to_concate.empty:
                    ...
                    to_concate = pd.merge(
                        to_concate, subset_df, how="outer", on="date, symbol"
                    )
            else:
...
if df is not None:
    ...
    for symbol in ticker_symbols:
        if symbol in df["symbol"].values:
            ...
            X_symbol, y_symbol = create_sequences(
                symbol_data[features].values, symbol_data[target].values, seq_length=60
            )
